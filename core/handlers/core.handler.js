...
// libreria
import $console from '../../library/utils/Logger.js'
...
export default async (messages, sock) => {
    ...
    for (const message of messages.messages) {
        ...
        /*Banned*/ {
            const isBot = m.sender.role('bot')

            const chat_db = await m.chat.db()
            if (chat_db.banned && !isBot) continue

            const user_db = await m.sender.db()
            if (user_db.banned && !isBot) continue
        }
        ...
        if (message.messageStubType) {
            ...
            if (plugins[0]) await plugins[0].script(m, {
                ...
                if (!global.config.silentConsole) {
        ...
        if (!global.config.silentConsole) {
            $console.log(chalk.white('['), chalk.magenta(moment().tz(Intl.DateTimeFormat().resolvedOptions().timeZone).format('HH:mm:ss')).trim(), chalk.white(']'), chalk.blue(`MENSAJE:`), chalk.green('{'), chalk.rgb(255, 131, 0).underline(m.content.text == '' ? (m.type[0] ? m.type : m.type[1]) : m.content.text), chalk.green('}'), chalk.blue('De'), chalk.cyan(m.sender.name), 'Chat', m.chat.isGroup ? chalk.bgGreen('grupo:' + (m.chat.name || m.chat.id)) : chalk.bgRed('Privado:' + (m.sender.roles.bot ? 'bot' : (m.sender.name || m.sender.id))))
        }
