...
export default async ({ m, sock }) => {
    const handlerDB = await $base.open('@reply:Handler')

    if (!handlerDB[m.quoted.id]) return

    const replyHandler = lodash.cloneDeep(handlerDB[m.quoted.id])
